...
ros::Publisher robotFrameCoordsPub;
pcl::PointCloud<pcl::PointXYZ>::Ptr pPCL;

void pointCloudTopicCallback(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pcl_msg)
{
    if (humanListMsg)
    {
        // ROS_INFO("pointCloudTopicCallback IN");
        pclMsg = true;
        pPCL = pcl_msg;
        humanListMsg = false;
        // ROS_INFO("pointCloudTopicCallback OUT");
    }
    else
    {
        ros::spinOnce();
...
void humanListCallback(const openpose_ros_msgs::OpenPoseHumanList::ConstPtr& list_msg)
{
    if (pclMsg)
    {
        ...
        pclMsg = false;
        // ROS_INFO("humanListCallback OUT");
    }
    else
    {
        ros::spinOnce();
